...
function loadCtyDat(filePath: string) {
  const content = fs.readFileSync(path.join(__dirname, filePath), 'utf-8')
  const lines = content.split(/\r?\n/)

  const prefixMap = new Map<string, Entity>()
  const exactMap = new Map<string, Entity>()
...
function lookup(target: string, percise = false): Entity | null {
  if (exactMap.has(target))
    return exactMap.get(target) || null
  // 精确匹配模式下不进行前缀查找
  if (percise) {
    return null
  }
...
export function getDXCC(callsign: string): Entity | null {
  if (!callsign)
    return null
