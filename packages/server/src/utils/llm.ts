...
const openai = new OpenAI({
  apiKey: 'sk-84610a2c484d499982c74b1961994e87', // 从环境变量读取
  baseURL: 'https://dashscope.aliyuncs.com/compatible-mode/v1',
})

async function detect(spot: Spot) {
  const freq = Number.parseFloat(spot.freq)
  const plan = BAND_PLAN.find(([min, max]) => freq >= min && freq < max) as [number, number, ...string[]] | undefined
  const modes = plan ? plan.slice(2) : []
  try {
    const messages: ChatCompletionMessageParam[] =  [{
        ...
        content: [
            ...
            '2. 判断通信模式是PH、DIGI还是CW。 通过Comment和频率相结合进行判断,一定要百分百确定才能判断，如果不能判断返回null。 ',
            '3. 判断要给出理由, 理由简短中文。',
            '请严格按照以下JSON格式返回结果：',
