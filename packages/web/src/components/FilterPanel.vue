...
function parseFrequency(freq: string) {
  const match = freq.match(/([\d.]+)\s*([a-z]+)/i)
  if (!match)
    return 0
...
const filterConfig = computed<FilterConfig>(() => {
  ...
  if (freqInclude.length > 0 || freqExclude.length > 0) {
    config.freqMarks = {}
    if (freqInclude.length > 0)
      config.freqMarks.include = freqInclude
    if (freqExclude.length > 0)
      config.freqMarks.exclude = freqExclude
  }
  ...
  if (specificInclude.length > 0 || specificExclude.length > 0) {
    config.specificFreq = {}
    if (specificInclude.length > 0)
      config.specificFreq.include = specificInclude
    if (specificExclude.length > 0)
      config.specificFreq.exclude = specificExclude
  }
  ...
  if (modeInclude.length > 0 || modeExclude.length > 0) {
    config.modeMarks = {}
    if (modeInclude.length > 0)
      config.modeMarks.include = modeInclude
    if (modeExclude.length > 0)
      config.modeMarks.exclude = modeExclude
  }
  ...
  if (dxInclude.length > 0 || dxExclude.length > 0) {
    config.dxMarks = {}
    if (dxInclude.length > 0)
      config.dxMarks.include = dxInclude
    if (dxExclude.length > 0)
      config.dxMarks.exclude = dxExclude
  }
    ...
    >
      <div class="flex flex-col h-[70vh]">
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
          ...
          <button class="text-gray-500 hover:text-gray-700 text-2xl leading-none" @click="closeDxccDialog">
            &times;
          </button>
        </div>
            ...
            >
              <div>
                ...
                <div class="text-xs opacity-70">
                  {{ entity.primary }} · {{ entity.continent }}
                </div>
              </div>
